...
# Find and include all .so.p directories containing generated headers (including subdirs)
for p_dir in $(find "$NATIVE_BUILD/pandas/_libs" -type d -name "*.p" 2>/dev/null); do
    WASI_CFLAGS="$WASI_CFLAGS -I$p_dir"
done

# Include subdirectories like window/
WASI_CFLAGS="$WASI_CFLAGS -I$NATIVE_BUILD/pandas/_libs/window"
...
# C++ specific flags
# Note: Cython-generated C++ uses exceptions, so we can't use -fno-exceptions
WASI_CXXFLAGS="$WASI_CFLAGS -std=c++17 -fno-rtti"
...
compile_cpp_file() {
    ...
    local err_output
    if err_output=$($WASI_CXX $WASI_CXXFLAGS $extra_flags -c "$src" -o "$obj" 2>&1); then
        echo "  ✓ $(basename $src)"
        return 0
    else
        # Check if file exists despite non-zero exit (warnings treated as errors)
        if [ -f "$obj" ]; then
            echo "  ✓ $(basename $src)"
            return 0
        else
            echo "  ✗ $(basename $src)"
            echo "$err_output" | head -10
            return 1
        fi
    fi
}
...
# Also look in subdirectories like window/
for p_dir in $(find "$LIBS_BUILD" -type d -name "*.so.p" 2>/dev/null); do
    module_name=$(basename "$p_dir" | sed 's/\.cpython.*//' | sed 's/\..*//')
    # Show relative path for nested modules
    rel_path=$(echo "$p_dir" | sed "s|.*$LIBS_BUILD/||")
    echo "  Module: $rel_path -> $module_name"
    # Find all .c files recursively in the .so.p directory
    for src in $(find "$p_dir" -name "*.c" -type f 2>/dev/null); do
        compile_file "$src" || true
    done
    # Find all .cpp files recursively in the .so.p directory
    for src in $(find "$p_dir" -name "*.cpp" -type f 2>/dev/null); do
...
# Compile C++ exception stubs (needed for aggregations module)
echo ""
echo "Compiling C++ exception stubs..."
if [ -f "$DEPS_DIR/wasi-stubs/cxx_exception_stubs.c" ]; then
    compile_file "$DEPS_DIR/wasi-stubs/cxx_exception_stubs.c" || true
fi
