/**
 * Python WASM module entry point for bundled projects with lxml support.
 *
 * This file is used when building Python projects that require the lxml
 * C extension. It registers lxml.etree with PyImport_AppendInittab before
...
// Forward declaration for lxml.etree init function
...
// Register C extension modules before Python initialization
static int register_extensions(void) {
    // Register lxml.etree as a built-in module
    if (PyImport_AppendInittab("lxml.etree", PyInit_etree) == -1) {
        fprintf(stderr, "Failed to register lxml.etree extension\n");
        return 1;
    }
    return 0;
}
